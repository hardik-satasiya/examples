...
import { addItemButtonClicked, addItemInputChanged, addItemInputEntered, clearCompletedButtonClicked, TodoAppAction } from "./actions";

export type Props = {

} & BaseApplicationProps;

export default (Base: React.ComponentClass<BaseApplicationProps>) => class ApplicationController extends React.PureComponent<Props, RootState> {
  state: RootState = {
    todoItems: [],
  };
  ...
  onInputKeyDown = (event: React.KeyboardEvent<any>) => {
    if (event.key === "Enter") {
      ...
      setTimeout(() => {
        this.dispatch(addItemInputChanged((nativeEvent.target as HTMLInputElement).value));
      }, 10);
    }
  }
  onAddItemClick = () => {
  ...
  render() {
    ...
    const { onAddItemClick, onClearCompletedClick, onInputKeyDown, dispatch} = this;

    const todoItemChildren = todoItems.map(item => {
      return <ListItem key={item.id} item={item} dispatch={dispatch} />
    })

    return <Base {...rest} inputProps={{
