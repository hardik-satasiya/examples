...
import { TodoAppAction, todoInputEntered, todoFilterItemButtonClicked } from "./actions";
...
export default (Base: React.ComponentClass<BaseAppProps>) => class AppController extends React.PureComponent<Props, RootState> {
  state: RootState = {
    todos: [],
  ...
  onInputKeyDown = (event: React.KeyboardEvent<any>) => {
    if (event.key === "Enter") {
      this.dispatch(todoInputEntered((event.target as HTMLInputElement).value));
    }
  }
  ...
  render() {
    ...
    return <Base {...rest} todoInputProps={{
