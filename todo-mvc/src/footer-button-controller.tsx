...
export default (Base: React.ComponentClass<BaseFooterButtonProps>) => class FooterButtonController extends React.PureComponent<Props> {
  onClick = () => {
  ...
  render() {
    ...
    const {onClick} = this;
    return <Base {...rest} variant={cx({active: selected})} onClick={onClick} labelTextProps={{text: label}} />;
  }
}